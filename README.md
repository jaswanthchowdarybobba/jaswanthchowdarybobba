<div align="center">

# 👋Hi, I'm Jaswanth Chowdary

### 🚀 Enterprise SharePoint Architect | Power Platform Specialist | SPFx (React) Expert

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jaswanthchowdarybobba/)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:bobbajasswanthchowdary@gmail.com)
[![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?style=for-the-badge&logo=microsoft-office&logoColor=white)](https://www.microsoft.com)

</div>
 
---

 ## ⚡ TL;DR

- 🚀 Built intranet platforms serving **500+ daily users**
- ⚡ Improved performance by **~40% (8s → 4.8s)**
- 📈 Increased engagement by **65% in 60 days**
- 🔄 Automated **20+ business workflows** (3 days → under 4 hours)
- 📊 Delivered **Power BI dashboards** used by leadership

---

## 🧠 About Me
 
> **SharePoint & Power Platform Developer** with **5+ years** of enterprise-scale Microsoft 365 experience — building intranet portals, automating workflows, and migrating legacy environments for **500+ user organizations**.
 
- 🏗️ Specialized in **SPFx (React + TypeScript)** custom web parts and solutions
- ⚡ Automated **20+ business processes** via Power Automate — cutting approval cycles from **3 days → under 4 hours**
- ☁️ Executed **3 zero-data-loss SharePoint migrations** (2010/2013 → Online)
- 📊 Delivered **Power BI dashboards** adopted as primary reporting tools by senior leadership
- 🔗 Deep expertise in **Microsoft Graph API** for live org data, calendar integration & more
- 📍 Based in India · Open to remote enterprise projects
 
---

## 🏆 Featured Project
 
### 🏢 Intranet Portal with Dashboard & Analytics
> *SharePoint Online · SPFx · React · TypeScript · Power BI · MS Graph API · Azure Functions · Fluent UI*
 
Enterprise intranet serving **500+ employees daily**, built from the ground up on SharePoint Online with SPFx.
 
- ⚡ **~40% faster** page load vs. legacy SharePoint 2013
- 📊 **8 custom SPFx web parts**: News Feed, Department Hub, Quick Links, Event Calendar, KPI Tracker, Org Directory, Weather, Announcements
- 📈 **5 embedded Power BI dashboards** (Finance, HR, IT, Ops, Sales) — adopted as primary reporting by leadership
- 📈 **~65% engagement increase** within 60 days of launch
  
🔗 [View Project](https://github.com/jaswanthchowdarybobba/enterprise-intranet-portal)

---

## 🛠️ Tech Stack
 
### SharePoint & Microsoft 365
![SharePoint](https://img.shields.io/badge/SharePoint-0078D4?style=flat-square&logo=microsoft-sharepoint&logoColor=white)
![SPFx](https://img.shields.io/badge/SPFx-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?style=flat-square&logo=microsoft-office&logoColor=white)
![Azure AD](https://img.shields.io/badge/Azure_AD-0089D6?style=flat-square&logo=microsoft-azure&logoColor=white)
![Azure Functions](https://img.shields.io/badge/Azure_Functions-0062AD?style=flat-square&logo=azure-functions&logoColor=white)
![Azure DevOps](https://img.shields.io/badge/Azure_DevOps-0078D7?style=flat-square&logo=azure-devops&logoColor=white)
 
### Power Platform
![Power Apps](https://img.shields.io/badge/Power_Apps-742774?style=flat-square&logo=powerapps&logoColor=white)
![Power Automate](https://img.shields.io/badge/Power_Automate-0066FF?style=flat-square&logo=powerautomate&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
 
### Development
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![Fluent UI](https://img.shields.io/badge/Fluent_UI-0078D4?style=flat-square&logo=microsoft&logoColor=white)
 
### APIs & Integration
![Microsoft Graph](https://img.shields.io/badge/Microsoft_Graph-00BCF2?style=flat-square&logo=microsoft&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-009688?style=flat-square&logo=fastapi&logoColor=white)
![PnP JS](https://img.shields.io/badge/PnP_JS-green?style=flat-square)
![CSOM](https://img.shields.io/badge/CSOM-0078D4?style=flat-square&logo=microsoft&logoColor=white)
 
### Migration & Governance
![Sharegate](https://img.shields.io/badge/Sharegate-FF6B00?style=flat-square)
![Metalogix](https://img.shields.io/badge/Metalogix-0078D4?style=flat-square)
![SPMT](https://img.shields.io/badge/SPMT-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![PnP PowerShell](https://img.shields.io/badge/PnP_PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
 
---
 
## 🚀 Key Projects
  
### 👩‍💼 Employee Service Portal
> *SPFx · React · TypeScript · Power Apps · Power Automate · MS Graph API · Fluent UI · PnP JS*
 
HR self-service portal with full Microsoft Graph API integration for live employee data.
 
- 🎟️ **5 custom SPFx web parts**: Leave Request, IT Ticket, Org Chart, Policy Library, Onboarding Tracker
- 🔄 Live employee profiles, reporting hierarchy & team calendar via Graph API — replaced static org chart
- ⏱️ Reduced HR request resolution from **5 days → 1 day**
- 👥 **300+ employees onboarded** with **85%+ adoption rate** within first month

🔗 [View Project](https://github.com/jaswanthchowdarybobba/employee-service-portal)
 
---
 
### 📂 Document Management System
> *SharePoint Online · Power Apps · Power Automate · Power BI · JSON · PnP PowerShell · Azure Runbook*
 
Metadata-driven DMS replacing manual document workflows with automated, compliant pipelines.
 
- 🗂️ **8 metadata-driven document libraries** with content types, versioning & retention
- ⏱️ **~60% faster** document retrieval through structured metadata taxonomy
- 🔄 **5 multi-stage approval workflows** — cycle time from 3 days → **under 4 hours**
- 📦 Automated archival pipeline processing **500+ documents/month** via Azure Runbook

🔗 [View Project](https://github.com/jaswanthchowdarybobba/document-management-system)
 
---
 
### ☁️ SharePoint Migration & Modernization
> *Sharegate · Metalogix · SPFx · Power Apps · Power Automate · CSOM · PnP PowerShell*
 
Full lifecycle migration of 3 legacy SharePoint environments to SharePoint Online.
 
- 📁 Migrated **50,000+ documents** and **200+ site pages** — zero data loss
- 📋 Converted **100%** of legacy InfoPath forms (12 total) → Power Apps Canvas Apps
- ♻️ Rebuilt 10 Nintex & SharePoint Designer workflows as Power Automate cloud flows — **~35% faster** execution
- 🔒 Enforced RBAC governance across **15+ site collections**

🔗 [View Project](https://github.com/jaswanthchowdarybobba/migration-modernization)

---

## 📊 Impact Snapshot

- 👥 500+ daily users
- ⚡ 40% performance improvement
- 📈 65% engagement growth
- 🔄 20+ automated workflows

---
 
## 📜 Certifications
 
| Certification | Issuer |
|---|---|
| 🏅 MS Power Platform Fundamentals | Microsoft |
| 🏅 SharePoint Online & M365 Admin | Microsoft |
 
---
  
## 🎯 What I Build
 
```
📌 SPFx Custom Web Parts & Extensions   →   React + TypeScript + Fluent UI
📌 Power Automate Workflows             →   Multi-stage approvals, HTTP actions, Graph API
📌 Power Apps Canvas Apps               →   Form replacements, self-service portals
📌 SharePoint Migrations                →   2010/2013/2016 → Online, zero data loss
📌 Microsoft Graph API Integrations     →   Org data, calendar, people, teams
📌 Power BI Dashboards                  →   Embedded in SharePoint, live KPI tracking
📌 Azure Functions                      →   Serverless backend for SPFx solutions
📌 Enterprise RBAC & Governance         →   15+ site collections, compliance policies
```
 
---
 
## 💬 Contact
 
<div align="center">
 
 
[![LinkedIn](https://img.shields.io/badge/Let's_Connect_on_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jaswanthchowdarybobba/)
[![Email](https://img.shields.io/badge/Send_an_Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:bobbajasswanthchowdary@gmail.com)
 
</div>
 
---
 
<div align="center">
 
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00BCF2,100:0078D4&height=100&section=footer" />
 
💡 *Building enterprise SharePoint solutions that people actually use.*
 
</div>
